...
tags: IDOR
source: https://tryhackme.com/room/corridor
difficulty: easy
---

![0xskar](/assets/corridor.jpg)

* * *

## Corridor

nmap returns a website with a corridor 

view sourcef for the html gives us an image map which maps out all the doors we can check out
...
These all return us to empty rooms with nothing in them. But the hint says follow the hashes and these seem to be md5 hashes. Maybe I can convert a medium directory list to md5 hashes and use that to find an open corridor.

Bash script to read and output the file

- `while read -r line; do printf %s "$line" | md5sum | cut -f1 -d' '; done < /usr/share/dirbuster/wordlists/directory-list-2.3-medium.txt >> output.txt`

We can then scan for open rooms.

- `feroxbuster -u http://10.10.80.97 -w corridor/output.txt`
